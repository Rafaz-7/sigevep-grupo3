...
def agregar(supabase):
    ...
    try:
        ...
        supabase.rpc('sp_insertar_asignacion', {'p_id_vol': id_voluntario, 'p_id_req': id_requerimiento, 'p_id_aprobador': id_aprobador, 'p_hora_ini': hora_inicio, 'p_hora_fin': hora_fin, 'p_estado': estado}).execute()
        console.print("[green]Asignación agregada exitosamente (Operación ejecutada via SP).[/green]")
    except Exception as e:
...
def editar(supabase):
    ...
    try:
        ...
        supabase.rpc('sp_actualizar_asignacion', {'p_id': id_asig, 'p_hora_ini': n_inicio, 'p_hora_fin': n_fin, 'p_estado': n_estado, 'p_justificacion': n_just, 'p_fecha_cancel': n_fecha}).execute()
        console.print("[green]Asignación actualizada exitosamente (Operación ejecutada via SP).[/green]")
    except Exception as e:
...
def eliminar(supabase):
    ...
    try:
        if confirmar("¿Está seguro de eliminar esta asignación?"):
            supabase.rpc('sp_eliminar_asignacion', {'p_id': id_asig}).execute()
            console.print("[green]Asignación eliminada (Operación ejecutada via SP).[/green]")
        else:
