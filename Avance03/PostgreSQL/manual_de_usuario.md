...
- **Cuenta de Supabase** con el schema ejecutado
...
### 2.3 Configurar conexión a Supabase
1. Renombrar `.env.example` a `.env`
2. Editar `.env` con las credenciales de su proyecto Supabase:
```env
SUPABASE_URL=https://su-proyecto.supabase.co
SUPABASE_KEY=su-api-key-aqui
```

### 2.4 Ejecutar el schema SQL
1. Abrir el SQL Editor en el dashboard de Supabase
2. Copiar y pegar el contenido completo de `schema_final.sql`
...
Los reportes están implementados como vistas SQL accesibles desde el SQL Editor de Supabase:
...
| Problema | Solución |
|----------|----------|
| `Error: getaddrinfo failed` | El proyecto de Supabase está pausado. Reactivarlo desde el dashboard. |
...
| `Error al eliminar: violates foreign key` | El registro tiene dependencias. Eliminar primero los registros dependientes. |
| `ModuleNotFoundError` | Ejecutar `pip install supabase rich python-dotenv`. |
...
├── db.py                  # Módulo de conexión a Supabase (Singleton)
