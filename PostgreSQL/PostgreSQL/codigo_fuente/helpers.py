...
def seleccionar_fk(supabase, tabla, campo_id, campo_display, prompt="Seleccione"):
    """Muestra registros de una tabla FK y pide seleccionar uno por ID."""
    try:
        data = supabase.table(tabla).select(f"{campo_id}, {campo_display}").order(campo_id).execute().data
    except Exception:
        data = []
    if not data:
