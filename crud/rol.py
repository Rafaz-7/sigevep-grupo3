...
def crear(supabase):
    ...
    try:
        ...
        datos_insert = {
            ...
            "requiere_epp": requiere_epp,
            "nivel_demanda_fisica": demanda
        }

        respuesta = supabase.table("rol").insert(datos_insert).execute()
        if respuesta.data:
            console.print("[bold green]Rol creado exitosamente.[/bold green]")
        else:
            console.print("[bold red]No se pudo crear el rol.[/bold red]")
    except Exception as e:
...
def editar(supabase):
    ...
    try:
        ...
        # Como pedir_edicion por defecto retorna string, manejamos bool de forma custom o con pedir
        ...
        datos_update = {
            ...
            "descripcion_rol": desc_nueva,
            "requiere_epp": req_epp_nuevo,
            "nivel_demanda_fisica": demanda_nueva
        }

        respuesta_update = supabase.table("rol").update(datos_update).eq("id_rol", id_rol).execute()
        if respuesta_update.data:
            console.print("[bold green]Rol actualizado exitosamente.[/bold green]")
        else:
            console.print("[bold red]No se pudo actualizar el rol.[/bold red]")
    except Exception as e:
...
def eliminar(supabase):
    ...
    try:
        ...
        if confirmar(f"¿Está seguro que desea eliminar el rol '{rol['nombre_rol']}'?"):
            res_delete = supabase.table("rol").delete().eq("id_rol", id_rol).execute()
            if res_delete.data:
                console.print("[bold green]Rol eliminado exitosamente.[/bold green]")
            else:
                console.print("[bold red]No se pudo eliminar el rol.[/bold red]")
        else:
